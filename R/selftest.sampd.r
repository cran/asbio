...
tkgrid(tklabel(tt, text = "If the parent distribution is \u0059 ~ (\u03bc, \u03c3\u00b2), \nthen the sampling distribution of \u0232 will be: "), columnspan = 2)
tkgrid(tklabel(tt,text=""))
tkgrid(tklabel(tt,text="  a.    \u0232 ~ (\u0232, \u03c3\u00b2).          "),rb1, sticky = "w")
tkgrid(tklabel(tt,text="  b.    \u0232 ~ (\u03bc, \u03c3/n).              "),rb2, sticky = "w")
tkgrid(tklabel(tt,text="  c.    \u0232 ~ (\u03bc/n, \u03c3\u00b2).        "),rb3, sticky = "w")
tkgrid(tklabel(tt,text="  d.    \u0232 ~ (\u03bc, \u03c3\u00b2/n).        "),rb4, sticky = "w")
tkgrid(tklabel(tt,text=""))
OnOK <- function()
{
...
tkgrid(tklabel(tt,text=""))
tkgrid(tklabel(tt,text="  a.    If Y ~ N(\u0232, \u03c3\u00b2), then \u0232 ~ N(\u03bc, \u03c3\u00b2/n)."),rb1, sticky = "w")
...
OnOK <- function()
{
    rbVal <- as.character(tclvalue(rbValue))
    if (rbVal=="a")
    	tkmessageBox(message="Partially correct.", icon = "error")
    if (rbVal=="b")
    	tkmessageBox(message="Partially correct.", icon = "error")
    if (rbVal=="c")
    	tkmessageBox(message="Partially correct.", icon = "error")
