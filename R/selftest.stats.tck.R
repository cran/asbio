...
selftest.stats.tck1 <- function(){
  ...
  f1 <- function(h,....){
    if(tail(svalue(r1),1) == ans1[1]){
      gmessage(msg="Incorrect. From the text in the question, we have the transform: X' = 2(3 + X). Based on this order of operations, we have: E(X') = 2(3 + 2) = 10.", icon = "error")
    }
    if(tail(svalue(r1),1)== ans1[2]){
      gmessage(msg="Incorrect", icon = "error")
    }
    if(tail(svalue(r1),1)== ans1[3]){
      gmessage(msg="Correct. From the text in the question, we have the transform: X' = 2(3 + X). Based on this order of operations, we have: E(X') = 2(3 + 2) = 10.")
    }
    if(tail(svalue(r1),1)== ans1[4]){
      gmessage(msg="Incorrect", icon = "error")
    }
