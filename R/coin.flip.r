...
coin.flip<-function(flips=1000,p.head=.5,interval=0.01,show.coin=TRUE,...){
  ...
  layout(matrix(c(rep(1,6),0,2,0), 3, 3, byrow = TRUE))	
    for(i in 1:flips){
        ...
        plot(seq(0,1),seq(0,1),type="n",xaxt="n",yaxt="n",xlab="",ylab="",bty="n")
            draw.circle(x=0.5,y=0.5,r=.39,col=rgb(blue=0.7,red=0.7,green=0.7,alpha=.8))
            text(0.5,0.5,ifelse(res[i]==0,"TAIL","HEAD"),cex=2.5)
            Sys.sleep(interval)
	}
    }
}
