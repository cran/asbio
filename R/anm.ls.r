        ...
        italic(E), "(", italic(X), ")", sep = "")), ...)
{
  ...
  if (is.null(poss))
    ...
    LS.est <- mean(X)
    if (plot.lsfunc == TRUE) {
      dev.new(height = ifelse(plot.res == TRUE, 4, NULL),
              width = ifelse(plot.res == TRUE, 8, NULL), noRStudioGD = TRUE)
        nm <- which(poss == est)[1]
        if (plot.res == TRUE) {
        ...
        if (anim == TRUE) {
         # if(options("device")$device == "RStudioGD") dev.new(noRStudioGD = TRUE)
            for (i in 1:nm) {
              dev.hold()
