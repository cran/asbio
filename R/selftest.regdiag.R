selftest.regdiag.tck1 <- function ()
{
  ...
  qq <- glabel("For general linear regression models, we assume...",
               container = gp1.1, anchor = c(-1, 1))
  font(qq) <- list(family = "cambria", size = 11)
  ans1 <- c("(a) Y\u1D62 ~ N(\u03BC, \u03C3\u00B2).", "(b) X\u1D62 ~ N(\u03BC, \u03C3\u00B2), for all predictors.",
            "(c) \u03B5\u1D62 ~ N(\u03BC, \u03C3\u00B2).", "(d) All of the above.")
  f1 <- function(h, ....) {
  ...
  r1 <- gcheckboxgroup(ans1, container = gp1, checked = FALSE,
                       where = "beginning", handler = f1)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("The diagnostic plot below indicates problems with...",
  ...
  img <- tkrplot::tkrplot(gp2.1, function() {
    ...
    plot(Fits, Residuals, xlab = "Fitted value", ylab = "Residual",
         cex.lab = ifelse(OS == "Linux", 1, 0.9))
    abline(h = 0, lty = 2, col = 2)
  })
  add(gp2, img, horizontal = TRUE)
  ans2 <- c("(a) non-normality.", "(b) heteroscedasticy.",
            "(c) non-linearity.", "(d) non-independence.", "(e) outliers.")
  f2 <- function(h, ....) {
  ...
  r2 <- gcheckboxgroup(ans2, container = gp2, checked = FALSE,
                       where = "beginning", handler = f2)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("The diagnostic plot below most strongly indicates problems with...",
               container = gp3.1, anchor = c(-1, 1))
  font(qq) <- list(family = "cambria", size = 11)
  x <- c(0.08, 0.92, 1.01, 1.1, 0.57, 0.11, 2.44, 0.88, 0.93,
         1.21, 4.01, 2.78, 0.65, 2.45, 0.9, 2.58, 0.76, 0.05,
         0.43, 0.42, 0.45, 0.56, 0.03, 0.83, 3.43, 0.06, 0.7,
         0.02, 1.97, 0.37, 2.19, 2.8, 0.55, 1.02, 0.46, 0.37,
         1.36, 1.14, 0.72, 0.01, 2.54, 1.88, 0.6, 1.12, 0.68,
         0.66, 0.16, 0.28, 0.54, 0.41, 0.04, 0.37, 0.44, 0.54,
         0.88, 0.43, 1.26, 0.86, 0.04, 2.29)
  gp3.1 <- getWidget(gp3)
  img <- tkrplot::tkrplot(gp3.1, function() {
    par(bg = "white", mar = c(4.5, 4.1, 1, 1))
    qqnorm(x, cex.lab = ifelse(OS == "Linux", 1, 0.9), main = "")
    qqline(x, col = 2, lty = 2)
  })
  add(gp3, img)
  ans3 <- c("(a) non-normality.", "(b) heteroscedasticy.",
            "(c) non-linearity.", "(d) non-independence.", "(e) outliers.")
  f3 <- function(h, ....) {
  ...
  r3 <- gcheckboxgroup(ans3, container = gp3, checked = FALSE,
                       where = "beginning", handler = f3)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("The plot below most strongly indicates problems with...",
  ...
  add(gp4, img)
  ans4 <- c("(a) non-normality.", "(b) heteroscedasticy.",
            "(c) non-linearity.", "(d) non-independence.", "(e) outliers.")
  f4 <- function(h, ....) {
  ...
  r4 <- gcheckboxgroup(ans4, container = gp4, checked = FALSE,
                       where = "beginning", handler = f4)
  ...
  img <- tkrplot::tkrplot(gp5.1, function() {
    par(bg = "white", mar = c(4.5, 4.1, 1, 1))
    plot(Fits, Residuals, xlab = "Order that data were collected",
         ylab = "Residual", cex.lab = ifelse(OS == "Linux", 1, 0.9))
    abline(h = 0, lty = 2, col = 2)
  })
  add(gp5, img, horizontal = TRUE)
  ans5 <- c("(a) non-normality.", "(b) heteroscedasticy.",
            "(c) non-linearity.", "(d) non-independence.", "(e) outliers.")
  f5 <- function(h, ....) {
  ...
  r5 <- gcheckboxgroup(ans5, container = gp5, checked = FALSE,
                       where = "beginning", handler = f5)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("The plot of data below indicate potential problems with...",
               container = gp6.1, anchor = c(-1, 1))
  font(qq) <- list(family = "cambria", size = 11)
  substrate <- c(0.01, 2.5, 5, 10, 20, 30, 40, 60, 90, 120)
  rate <- c(0.373633, 7.0338, 12.84083, 18.355, 23.47183,
            31.42908, 28.82768, 34.36157, 38.0748, 38.1663)
  gp6.1 <- getWidget(gp6)
  img <- tkrplot::tkrplot(gp6.1, function() {
    par(bg = "white", mar = c(4.5, 4.1, 1, 1))
    plot(substrate, rate, xlab = expression(paste("Substrate (",
                                                  mu, "mols)", sep = "")),
         ylab = expression(paste("Lipid breakdown (nmol/mg enzyme)", sep = "")),
         cex.lab = ifelse(OS == "Linux", 1, 0.9))
  })
  add(gp6, img, horizontal = TRUE)
  ans6 <- c("(a) non-normality.", "(b) heteroscedasticy.",
            "(c) non-linearity.", "(d) non-independence.", "(e) outliers.")
  f6 <- function(h, ....) {
  ...
  r6 <- gcheckboxgroup(ans6, container = gp6, checked = FALSE,
                       where = "beginning", handler = f6)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("Cook's distance...", container = gp7.1, anchor = c(-1,
                                                                   1))
  font(qq) <- list(family = "cambria", size = 11)
  ...
  r7 <- gcheckboxgroup(ans7, container = gp7, checked = FALSE,
                       where = "beginning", handler = f7)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("Leverage...", container = gp8.1, anchor = c(-1,
                                                            1))
  font(qq) <- list(family = "cambria", size = 11)
  ...
  r8 <- gcheckboxgroup(ans8, container = gp8, checked = FALSE,
                       where = "beginning", handler = f8)
  ...
  font(q) <- list(weight = "bold")
  qq <- glabel("A Variance Inflation Factor (VIF)...", container = gp9.1,
               anchor = c(-1, 1))
  font(qq) <- list(family = "cambria", size = 11)
  ...
  r9 <- gcheckboxgroup(ans9, container = gp9, checked = FALSE,
                       where = "beginning", handler = f9)

  #--------------- Question 10 -----------------#

  gp10 <- gframe(container = g, spacing = 2, pos = 0, horizontal = FALSE)
  gp10.1 <- ggroup(container = gp10, spacing = 2, pos = 0,
                   horizontal = TRUE)
  q <- glabel("10) ", container = gp10.1, horizontal = TRUE)
  font(q) <- list(weight = "bold")
  qq <- glabel("We use the Box-Tidwell procedure to obtain optimal power transformation(s)...",
  ...
  r10 <- gcheckboxgroup(ans10, container = gp10, checked = FALSE,
                        where = "beginning", handler = f10)
  gp11 <- gframe(container = g, spacing = 2, pos = 0, horizontal = FALSE)
  gp11.1 <- ggroup(container = gp11, spacing = 2, pos = 0,
                   horizontal = TRUE)
  q <- glabel("11) ", container = gp11.1, horizontal = TRUE)
  font(q) <- list(weight = "bold")
  qq <- glabel("We use the Box-Cox procedure to obtain optimal power transformation(s)...",
  ...
  r11 <- gcheckboxgroup(ans11, container = gp11, checked = FALSE,
                        where = "beginning", handler = f11)
}
