see.anova.tck <- function () 
{
    ...
    norm.refresh <- function(...) {
        ...
        cols <- c(rgb(red=0.4,blue=0.4,green=0.5,alpha=.6), rgb(red=0.4,blue=0.8,green=0.5,alpha=.6),rgb(red=0.8,blue=0.4,green=0.5,alpha=.6))
        
        curve(dnorm(x,mu1,sigma),from=-5,to=5,ylim=ylim,ylab="f(x)")
        title(cex.main = 1, font.main = 1, main = paste("ANOVA from random sample of populations:", "\n", 
        "MSA = ", round(MSA,2), "  MSE = ", round(MSE, 2), "  F* = ", round(F.star, 2), "  P-value = ", round(P, 5)))
        ...
        polygon(c(xx[xx<=10],10),c(yy1[xx<=10],yy1[xx==-10]),col=cols[1])
        polygon(c(xx[xx<=10],10),c(yy2[xx<=10],yy2[xx==-10]),col=cols[2])
        polygon(c(xx[xx<=10],10),c(yy3[xx<=10],yy3[xx==-10]),col=cols[3])
               
        
        legend("topright", pch=22, pt.cex=1.7, pt.bg=cols, legend = c(expression(X[1]), expression(X[2]), expression(X[3])), bty = "n")
        
        legend("topleft", legend = c(expression(paste(alpha[1], "  =")), expression(paste(alpha[2], "  =")), expression(paste(alpha[3], "  =")),"", expression(paste(italic(E),"(", italic(MSA), ") = ")), expression(paste(italic(E),"(", italic(MSE), ") = "))), bty = "n")
        legend(-4.7, 0.833, legend = c(round(mu1-mu, 2), round(mu2 - mu, 2), round(mu3 - mu, 2), "", paste("            ", round(E.MSA, 2)), paste("            ", round(sigma^2, 2))), bty = "n")
