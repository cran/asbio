...
draw.cor.range <- function(y1, y2, a = 0, b = 2){
dev.hold()
plot(y1, y2, xlab=expression(Y[1]),ylab=expression(Y[2]), type = "n")
rect(a,-10, b, 10, col = rgb(red=0.4,blue=0.4,green=0.5,alpha=.9))
ny1 <- c(y1[y1 >= a & y1 <= b])
ny2 <- c(y2[y1 >= a & y1 <= b])
points(y1, y2)
points(ny1, ny2, pch = 19)
legend("topleft", bty = "n", title = expression(paste("     r        ",  r[k], "       ",  r[s])), legend = c(paste(round(cor(y1, y2), 2),round(cor(y1, y2, method = "k"), 2), round(cor(y1, y2, method = "s"), 2)),
...
require(tcltk) || stop("tcltk support is absent")
    ...
    norm.refresh <- function(...) {
        ...
        draw.cor.range(y1 = y1, y2 = y2, as.numeric(a), as.numeric(b))
        }
