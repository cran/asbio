samp.dist<-function(parent, parent2 = parent, s.size = 1, s.size2 = s.size, n.seq=seq(1,30), 
    ...
    func = NULL, show.n = TRUE, show.SE = FALSE, est.density = TRUE, col.density = 4, lwd.density = 2, 
    est.ylim = TRUE, anim = TRUE, interval = 0.01, col.anim = "rainbow", digits = 3,...){
    ...
    func = func, show.n = show.n, show.SE = show.SE, anim = anim, interval = interval, col.anim = col.anim, digits = digits,...) 

    if(fix.n==FALSE)
    samp.dist.n(parent=parent, R = R, n.seq = n.seq, 
    stat = stat, xlab = xlab, ylab = ylab, breaks = breaks, ylim = ylim,
    func = func, show.n = show.n, show.SE = show.SE, est.density = est.density, col.density = col.density, lwd.density = lwd.density, 
    est.ylim = est.ylim, anim = anim, interval = interval, col.anim = col.anim, digits = digits,...)
}
...
samp.dist.fixn<-function (parent, parent2 = parent, s.size = 1, s.size2 = s.size, 
    ...
    func = NULL, show.n=TRUE,show.SE=FALSE,anim=TRUE,interval=0.01,col.anim="rainbow", digits = 3,...){ 
    ...
    if(show.n==TRUE&show.SE==FALSE){legend("topright", legend = paste("n = ", s.size), 
    bty = "n")}
    if(show.SE==TRUE&show.n==FALSE){legend("topright", legend = paste("SE = ", SE), 
    bty = "n")}
    if(show.SE==TRUE&show.n==TRUE){legend("topright", legend = c(paste("n = ", s.size),paste("SE = ", SE)), 
    bty = "n")}  
}
...
samp.dist.n<-function (parent, R = 500, n.seq=seq(1,30), 
    ...
    func = NULL, show.n=TRUE, show.SE =FALSE, est.density = TRUE, col.density = 4,  lwd.density = 2, est.ylim = TRUE, ylim = NULL, anim = TRUE, interval=0.5, col.anim = NULL,digits=3,...){ 
    ...
    if(n.min==1&is.na(stat(1)))stop("Statistic in stat cannot be computed with n = 1, revise n.seq.") 
       ...
       if(anim==FALSE){
        ...
        if(show.n==TRUE&show.SE==FALSE){legend("topright", legend = paste("n = ", n.max),bty = "n")}
        if(show.SE==TRUE&show.n==FALSE){legend("topright", legend = paste("SE = ", SE),bty = "n")} 
        if(show.SE==TRUE&show.n==TRUE){legend("topright", legend = c(paste("n = ", n.max),paste("SE = ", SE)),bty = "n")}
            }     
       if(anim==TRUE){
           if(is.null(col.anim))clr=NULL
            if(!is.null(col.anim)){
            ...
            for(i in 1:length(n.seq)){
                ...
                if(show.n==TRUE&show.SE==FALSE){legend("topright", legend = paste("n = ", n.seq[i]), 
                    bty = "n")}
                if(show.SE==TRUE&show.n==FALSE){legend("topright", legend = paste("SE = ", SE[i]), 
                    bty = "n")}
                if(show.SE==TRUE&show.n==TRUE){legend("topright", legend = c(paste("n = ", n.seq[i]),paste("SE = ", SE[i])), 
                    bty = "n")} 
                if(est.density==TRUE){
