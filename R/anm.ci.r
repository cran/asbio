...
for(i in 1:n.est){
    x<-sample(parent,size=n,replace=FALSE)
    if(par.type=="mu"){
      if(!is.null(sigma)){
 ...
 if(par.type=="median")mtext(names[2],3,at=par.val,font=3,line=.1)
 if(par.type=="sigma.sq")mtext(names[3],3,at=par.val,font=3,line=.1)
 if(par.type=="p")mtext(names[4],3,at=par.val,font=3,line=.1)
 ...
 mtext(bquote(paste("Conf. = ", .(conf))),3,at=min(ci[,2]),adj=0,line=1)
 }
