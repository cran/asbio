...
tkgrid(tklabel(tt,text=""))
tkgrid(tklabel(tt,text="  a.    A continuous response, e.g. height or weight.  "),rb1, sticky = "w")
...
tkgrid(tklabel(tt,text="  d.    A discrete response, e.g. height or weight.   "),rb4,sticky="w")
tkgrid(tklabel(tt,text=""))
OnOK <- function()
{
...
OnOK <- function()
{
    rbVal <- as.character(tclvalue(rbValue))
    if (rbVal=="a")
    	tkmessageBox(message="Incorrect.  The binomial pdf discribes discrete random variables.",icon="error")
    if (rbVal=="b")
    	tkmessageBox(message="Incorrect.  The upper bound will equal the number of trials, n",icon="error")
    if (rbVal=="c")
    	tkmessageBox(message="Correct.")
    if (rbVal=="d")
    	tkmessageBox(message="Incorrect.  The binomial pdf discribes describes discrete random variables (height and weight are not discrete).",icon="error")  	
}
