...
The term \eqn{r_hN_h} describes exponential growth for the host (prey) species.  This will be opposed by deaths due to predation, i.e. the term \eqn{pN_hN_p}.  The term \eqn{cpN_hN_p} is the rate at which predators destroy prey. This in turn will be opposed by \eqn{d_pN_p}, i.e. predator deaths.  . The functions \code{anm.LVe.tck()} and \code{anm.LVc.tck()} allow one to run \code{anm.LVe()} and \code{anm.LVc()}  with \pkg{tcltk} GUIs.
}
